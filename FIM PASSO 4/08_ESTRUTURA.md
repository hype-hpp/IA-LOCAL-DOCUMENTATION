...
Referência da estrutura real do projeto em `~/IA-LOCAL/`, como ficou ao final da Fase 04.
...
│   ├── build_sandbox.sh           # NOVO (Fase 04) — builda a imagem do sandbox
│   ├── generate_code.py           # NOVO (Fase 04) — CLI: gera código via Qwen3-Coder (com --run para já executar no sandbox)
│   └── solve_task.py              # NOVO (Fase 04) — CLI: loop completo (gera -> executa -> corrige -> executa de novo)
│
...
│   ├── test_sandbox_executor.py     # NOVO (Fase 04) — executor do sandbox: sucesso, erro, timeout, sem rede, limpeza (precisa Docker)
│   ├── test_coder_client_parsing.py # NOVO (Fase 04) — extract_code()/build_prompt(), sem rede
│   ├── test_agent_loop.py           # NOVO (Fase 04) — controle do loop de iteração, com fakes (sem Ollama/Docker)
│   └── test_coding_agent_e2e.py     # NOVO (Fase 04) — pipeline completo da Fase 04 de ponta a ponta (precisa Ollama + Docker)
│
...
| `src/` | Código de produção, importável. Cresce por área (`ingestion/`, `retrieval/`, `search/`, `sandbox/`, `coding/`, e futuramente `agent/`, `crawler/`, etc., conforme novas fases). |
