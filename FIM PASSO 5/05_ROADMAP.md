...
Entregue:
- Schema de metadata de memória em `global_scope` via campo `memory_type` (`knowledge`/`research`/`manual`), inteiramente no payload do Qdrant — sem componente novo (`src/memory/schema.py`, Decisions 036/037) — 5.1
- Mecanismo `/save`: promoção explícita `chat_id` + IDs específicos de `chat_scope` para `global_scope`, reaproveitando o vetor já existente (sem reembedding), cópia — não move (`src/memory/save.py` + `scripts/save_memory.py`, Decision 038) — 5.2
- Nota manual avulsa: texto direto via `--text`, `memory_type="manual"`, `source` fixo em `"manual"` (`src/memory/add_note.py` + `scripts/add_memory.py`, Decision 039) — 5.3
- Visualizar/editar/apagar memórias (regra 10 do projeto): listar com filtros (`memory_type`/tag/`chat_id`), ver detalhe, editar tags (mesmo ID) ou texto (ID novo, reembedding), apagar só por ID explícito com confirmação interativa (`src/memory/manage.py` + `scripts/list_memories.py`, `edit_memory.py`, `delete_memory.py`, Decision 040) — 5.4
- Teste de integração end-to-end cobrindo o pipeline completo da fase — evidência → `/save` → nota manual → listar → editar tags → editar texto → apagar — contra Qdrant e Ollama reais (`tests/test_memory_e2e.py`) — 5.5

Escopo explicitamente fora desta fase (Decision 042): "memória de conversa" fica pendente para quando existir Agent Core / loop de chat real (Fase 07); interface permanece CLI, não a UI Gradio/Streamlit prevista na Decision 019.
